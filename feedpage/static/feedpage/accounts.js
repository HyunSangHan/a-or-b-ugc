$(document).ready(() => {
  ...
  $("input:radio[name=gender]:input[value=" + genderValue + "]").attr(
    "checked",
    true
  );
  if (majorValue !== "") {
    $("input:radio[name=major]:input[value=" + majorValue + "]").attr(
      "checked",
      true
    );
  }
  $("input:radio[name=mobile]:input[value=" + mobileValue + "]").attr(
    "checked",
    true
  );
  $("input:radio[name=politics]:input[value=" + politicsValue + "]").attr(
    "checked",
    true
  );
  $("input:radio[name=religion]:input[value=" + religionValue + "]").attr(
    "checked",
    true
  );
  ...
  $(".img-prf").on("change", function() {
    ...
    if (window.FileReader && $this[0].files[0]) {
      ...
      reader.onload = function(e) {
        ...
        $profileDiv.prepend(
          `
        <div class="creator-img" style="background: url('` +
            img +
            `') center / cover;">
